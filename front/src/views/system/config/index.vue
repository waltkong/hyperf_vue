<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="组别" width="150px">
        <template slot-scope="{row}">
      ...
      <el-table-column label="值" min-width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.value }}</span>
        </template>
      </el-table-column>
      <el-table-column label="值类型" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.type }}</span>
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['id', '组别', '名称', '值', '值类型', '创建时间']
        const filterVal = ['id', 'group_name', 'name', 'value', 'type', 'created_at']
