<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="路由" min-width="200px">
        <template slot-scope="{row}">
          <span>{{ row.url }}</span>
        </template>
      </el-table-column>
      <el-table-column label="层级" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.level }}</span>
        </template>
      </el-table-column>
      <el-table-column label="权限认证" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.need_auth | needAuthFilter }}</span>
        </template>
      </el-table-column>
      <el-table-column label="是否菜单" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.is_menu | isMenuFilter}}</span>
        </template>
      </el-table-column>
      <el-table-column label="超管理员" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.is_only_super_admin | isSuperAdminFilter }}</span>
        </template>
      </el-table-column>
      <el-table-column label="超管公司" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.is_only_super_company | isSuperCompanyFilter}}</span>
        </template>
      </el-table-column>
...
<script>
  ...
  import { dataList, deleteOne } from '@/api/system/menu_api'
    ...
    methods: {
      ...
      handleDownload() {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['id', '名称', '路由', '创建时间']
          const filterVal = ['id', 'name', 'url', 'created_at']
